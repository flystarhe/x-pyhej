...
def train_epoch(train_loader, model, loss_fun, optimizer, train_meter, cur_epoch):
    ...
    for cur_iter, (inputs, labels) in enumerate(train_loader):
        ...
        loss, err_mean, err_var = dist.scaled_all_reduce([loss, err_mean, err_var])
        # Copy the stats from GPU to CPU (sync point)
        loss, err_mean, err_var = loss.item(), err_mean.item(), err_var.item()
        train_meter.iter_toc()
        # Update and log stats
        mb_size = inputs.size(0) * cfg.NUM_GPUS
        train_meter.update_stats(err_mean, err_var, loss, lr, mb_size)
